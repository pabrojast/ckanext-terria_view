...
- Los tokens del proxy se firman HMAC-SHA256 con `ckanext.terria_view.proxy_token_secret` (o, en su defecto, `beaker.session.secret`/`SECRET_KEY`; ver [[Variables de Entorno]]), expiran por defecto en 1h y están atados a un `resource_id` específico. Por sesion la respuesta es `private, no-store` + `Vary: Cookie`; por token `private, max-age=300`.
