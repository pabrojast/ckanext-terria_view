...
- endurecimiento: los catalogos publicos de organizacion y tag omiten datasets con `access_level` no publico (top-level o en `extras`); `cache/stats|invalidate|cleanup` devuelven `401` anonimo, `403` sin sysadmin y solo ejecutan con sysadmin; el token usa `proxy_token_secret` antes que `beaker.session.secret` y sin secreto falla cerrado;
